...
b0_start <- log(70/sf::st_area(sf::st_as_sf(S)))
s0_start <- log(0.2)
UDG_start <- data.frame(N = 5, R = 150/4300, e = 1, n = 1, theta = pi/2, sigma = 0.15)
lc_start <- log(1/sf::st_area(sf::st_as_sf(S)))
gal_start <- cbind(N = c(log(25), log(120)), R = c(6.5 - log(4400), 7 - log(4400)), n = c(0.5, 0.5))

# specify prior parameters
UDG_prior <- data.frame(N_mean = log(5), N_sd = 1, R_mean = -3.9, R_sd = 0.5,
                        e_mean = log(1.1), e_sd = 0.3, n_mean = 0, n_sd = 0.75)

b0_prior <- c(b0_start, 0.25)
lc_prior <- c(log(2), 0.2)
s0_prior <- c(0.2, 0.05)

gal_prior <- data.frame(N_mean = c(log(25), log(120)), N_sd = c(0.25, 0.25),
                        R_mean = c(6.5 - log(4400), 7 - log(4400)), R_sd = c(0.25, 0.25),
...
#save results
date <- Sys.Date()
filename <- paste0('data/v8wfc3-pcp-col-',date,'-seed-', seed, '-100k.RDS')
saveRDS(res_col, filename)

date <- Sys.Date()
filename <- paste0('data/v8wfc3-pcp-',date,'-seed-', seed, '-100k.RDS')
