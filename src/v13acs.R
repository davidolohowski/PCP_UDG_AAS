...
gal_prior <- data.frame(N_mean = c(log(20)), N_sd = c(0.25),
                        R_mean = c(4.6 - log(4300)), R_sd = c(0.25),
                        n_mean = c(log(0.5)), n_sd = c(0.5))

# fixed parameters for normal galaxies
gal_fix <- data.frame(x = c(3088)/4300, y = c(4132)/4300,
                      theta = c(pi/4), e = c(1.36))
...
#save results
date <- Sys.Date()
filename <- paste0('data/v13acs-pcp-col-',date,'-seed-', seed, '-100k.RDS')
saveRDS(res_col, filename)

date <- Sys.Date()
filename <- paste0('data/v13acs-pcp-',date,'-seed-', seed, '-100k.RDS')
