...
v_acs <- read_csv('data/v6wfc3_GC.csv')
col <- readRDS('data/v6_wfc3_col.RDS')
pts <- v_acs[,3:4]/4300

# chain for model 1
res <- readRDS('data/v6wfc3_res/v6wfc3-pcp-2022-07-27-seed-577902-100k.RDS')
# chain for model 2
res_col <- readRDS('data/v6wfc3_res/v6wfc3-pcp-col-2022-07-27-seed-577902-100k.RDS')
...
prior_df_ncol <- data.frame(value = c(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 
                            ...
                            density = c(dlnorm(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 3.5072283, 0.25),
...
prior_df_col <- data.frame(value = c(seq(min(col_mod$b0), max(col_mod$b0), length= 100), 
                            ...
                            density = c(dlnorm(seq(min(col_mod$b0), max(col_mod$b0), length= 100), 3.5072283, 0.25),
...
tikz(file = "v6wfc_post_ncol.tex", standAlone=T, width = 6, height = 3)
...
dev.off()
system('pdflatex v6wfc_post_ncol.tex')

tikz(file = "v6wfc_post_col.tex", standAlone=T, width = 9, height = 3)
...
dev.off()
system('pdflatex v6wfc_post_col.tex')

# Whether there are UDGs
