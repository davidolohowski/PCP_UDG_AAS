...
set.seed(977804)
U1_sim <- data.frame(x = rnorm(9, U1[1], 0.018), y = rnorm(9, U1[2], 0.018))
U2_sim <- data.frame(x = rnorm(6, U2[1], 0.015), y = rnorm(6, U2[2], 0.015))

pts <- v_acs %>%
  dplyr::select(x, y)/4300

pts <- pts[-c(U1_GC, U2_GC),]
pts <- rbind(pts, rbind(U1_sim, U2_sim))
plot(pts)
...
ggplot(X_nc, aes(x, y)) + stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE, n = 200, h = c(bw.nrd(X_nc$x), bw.nrd(X_nc$y))) + geom_point(data = U1_df, aes(x,y))
...
for (i in 1:100) {
  set.seed(i^3)
  U1_col <- rnorm(9, mean_U1_col, sd_U1_col)
  U2_col <- rnorm(6, mean_U2_col, sd_U2_col)
  
  color <- c(v_acs$color[-c(U1_GC, U2_GC)], U1_col, U2_col)
  ...
  output_file_sim <- paste0('PCP_N=9X6_sim', i, '.RDS')
