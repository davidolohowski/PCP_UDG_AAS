...
gal_start <- cbind(N = c(log(130)), R = c(6.8 - log(4300)), n = c(0.5))
...
gal_prior <- data.frame(N_mean = c(log(130)), N_sd = c(0.25),
                        R_mean = c(6.5 - log(4300)), R_sd = c(0.25),
...
#save results
date <- Sys.Date()
filename <- paste0('data/v14acs-pcp-col-',date,'-seed-', seed, '-100k.RDS')
saveRDS(res_col, filename)

date <- Sys.Date()
filename <- paste0('data/v14acs-pcp-',date,'-seed-', seed, '-100k.RDS')
