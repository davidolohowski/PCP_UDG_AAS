...
b0_start <- log(60/sf::st_area(sf::st_as_sf(S)))
s0_start <- log(0.25)
UDG_start <- data.frame(N = 10, R = 150/4300, e = 1, n = 1, theta = pi/2, sigma = 0.15)
lc_start <- log(1/sf::st_area(sf::st_as_sf(S)))
gal_start <- cbind(N = log(40), R = 5 - log(4300), n = 0.5)

# specify prior parameters
UDG_prior <- data.frame(N_mean = log(10), N_sd = 1, R_mean = -3.9, R_sd = 0.5,
...
#save results
date <- Sys.Date()
filename <- paste0('data/v10acs-pcp-col-',date,'-seed-', seed, '-100k.RDS')
saveRDS(res_col, filename)

date <- Sys.Date()
filename <- paste0('data/v10acs-pcp-',date,'-seed-', seed, '-100k.RDS')
