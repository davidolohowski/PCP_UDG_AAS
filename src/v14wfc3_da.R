...
# chain for model 1
res <- readRDS('data/v14wfc3_res/v14wfc3-pcp-2022-07-27-seed-209886-100k.RDS')
# chain for model 2
res_col <- readRDS('data/v14wfc3_res/v14wfc3-pcp-col-2022-07-27-seed-209886-100k.RDS')
...
prior_df_ncol <- data.frame(value = c(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 
                            ...
                            density = c(dlnorm(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 3.506711, 0.25),
                                        ...
                                        dlnorm(seq(min(ncol_mod$R1), max(ncol_mod$R1), length= 100), 4.9 - log(4400), 0.25)),
...
prior_df_col <- data.frame(value = c(seq(min(col_mod$b0), max(col_mod$b0), length= 100), 
                           ...
                           density = c(dlnorm(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 3.506711, 0.25),
                                       ...
                                       dlnorm(seq(min(ncol_mod$n1), max(ncol_mod$n1), length= 100), log(0.5), 0.5),
                                       dlnorm(seq(min(ncol_mod$N1), max(ncol_mod$N1), length= 100), log(10), 0.25),
                                       dlnorm(seq(min(ncol_mod$R1), max(ncol_mod$R1), length= 100), 4.9 - log(4300), 0.25),
...
tikz(file = "v14wfc_post_ncol.tex", standAlone=T, width = 9, height = 4.5)
...
dev.off()
system('pdflatex v14wfc_post_ncol.tex')

tikz(file = "v14wfc_post_col.tex", standAlone=T, width = 9, height = 4.5)
...
dev.off()
system('pdflatex v14wfc_post_col.tex')
