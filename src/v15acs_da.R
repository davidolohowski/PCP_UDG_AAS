...
# chain for model 1
res <- readRDS('data/v15acs_res/v15acs-pcp-2022-07-18-seed-745382-100k.RDS')

# chain for model 2
res_col <- readRDS('data/v15acs_res/v15acs-pcp-col-2022-07-18-seed-745382-100k.RDS')
...
prior_df_ncol <- data.frame(value = c(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 
                            ...
                            density = c(dlnorm(seq(min(ncol_mod$b0), max(ncol_mod$b0), length= 100), 3.5065822, 0.25),
                                        ...
                                        dlnorm(seq(min(ncol_mod$N1), max(ncol_mod$N1), length= 100), log(30), 0.25),
                                        dlnorm(seq(min(ncol_mod$n2), max(ncol_mod$n2), length= 100), log(0.5), 0.5),
                                        dlnorm(seq(min(ncol_mod$N2), max(ncol_mod$N2), length= 100), log(10), 0.25),
                                        dlnorm(seq(min(ncol_mod$R1), max(ncol_mod$R1), length= 100), 6.34 - log(4300), 0.2),
                                        dlnorm(seq(min(ncol_mod$R2), max(ncol_mod$R2), length= 100), 5.8 - log(4300), 0.2)),
...
prior_df_col <- data.frame(value = c(seq(min(col_mod$b0), max(col_mod$b0), length= 100), 
                           ...
                           density = c(dlnorm(seq(min(col_mod$b0), max(col_mod$b0), length= 100), 3.5065822, 0.25),
                                       ...
                                       dlnorm(seq(min(col_mod$N1), max(col_mod$N1), length= 100), log(30), 0.25),
                                       dlnorm(seq(min(col_mod$n2), max(col_mod$n2), length= 100), log(0.5), 0.5),
                                       dlnorm(seq(min(col_mod$N2), max(col_mod$N2), length= 100), log(10), 0.25),
                                       dlnorm(seq(min(col_mod$R1), max(col_mod$R1), length= 100), 6.34 - log(4300), 0.2),
                                       dlnorm(seq(min(col_mod$R2), max(col_mod$R2), length= 100), 5.8 - log(4300), 0.2),
...
tikz(file = "v15acs_post_ncol.tex", standAlone=T, width = 9, height = 9)
...
dev.off()
system('pdflatex v15acs_post_ncol.tex')

tikz(file = "v15acs_post_col.tex", standAlone=T, width = 9, height = 9)
...
dev.off()
system('pdflatex v15acs_post_col.tex')
...
U <- data.frame(x = 464, y = 3562)/4300
...
prior_df_ncol_U <- data.frame(value = c(seq(min(df_U_ncol$N), max(df_U_ncol$N), length= 100), 
                              ...
                              density = c(dlnorm(seq(min(df_U_ncol$N), max(df_U_ncol$N), length= 100), log(4), 1),
...
tikz(file = "v15acs_U_post_ncol.tex", standAlone=T, width = 9, height = 3)
...
dev.off()
system('pdflatex v15acs_U_post_ncol.tex')
...
prior_df_col_U <- data.frame(value = c(seq(min(df_U_col$N), max(df_U_col$N), length= 100), 
                             ...
                             density = c(dlnorm(seq(min(df_U_col$N), max(df_U_col$N), length= 100), log(4), 1),
...
tikz(file = "v15acs_U_post_col.tex", standAlone=T, width = 6, height = 6)
...
dev.off()
system('pdflatex v15acs_U_post_col.tex')

# density and probability plots
ncol_dat <- res[[1]][-c(1:10000)][seq(1, 90001, 5)]
col_dat <- res_col[[1]][-c(1:10000)][seq(1, 90001, 5)]
...
dm <- sapply(ci, function(x) detect_metric(U, 0.023/3, x, X_nc, X_c))
...
for(i in 1:4){
  ...
  for (j in 1:100) {
    ...
    X_c_j <- do.call(rbind, X_c_j)
    X_c_j <- as.data.frame(X_c_j)
    pm <- detect_metric(U, c(0.023/3), ci[i], X_nc_j, X_c_j)
    acc_bs[[i]][j] <- pm[[1]]
    FP_bs[[i]][j] <- pm[[2]]
    prec_bs[[i]][j] <- pm[[3]]
  }
...
saveRDS(list(dm, acc_bs, FP_bs, prec_bs), 'data/v15acs_res/v15acs_pm.RDS')
...
ggplot(dat_dm, aes(prob, prec)) + geom_line(size = 0.2, color = "#A25050") + 
  ...
  ylim(c(0.9,1.5)) + geom_abline(intercept = 1, slope = 0, linetype = 'dashed', size = 0.2) + theme_minimal() +
  xlab('$1-\\alpha$') + ylab('Prec(Model 1)/Prec(Model 2)')
  
options(tikzLatexPackages 
        =c(getOption( "tikzLatexPackages" ),"\\usepackage{amsfonts}"))

tikz(file = "v15acs_model1_res.tex", standAlone=T, width = 5, height = 4.5)
ggplot(C_post_r, aes(x, y)) +  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE, n = 200) + 
  ...
  xlab('X') + ylab('Y')
dev.off()
system('pdflatex v15acs_model1_res.tex')

tikz(file = "v15acs_model2_res.tex", standAlone=T, width = 6, height = 5)
ggplot(C_post_rc, aes(x, y)) +  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE, n = 200) + 
  ...
  xlab('X') + ylab('Y')
dev.off()
system('pdflatex v15acs_model2_res.tex')
